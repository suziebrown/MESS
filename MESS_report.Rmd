...
# Think about transforming some variables
...
summary(mycox)
```
Next we assess whether proportional hazards is a suitable assumption for these data. We see from the residual plots that the treatment seems to have a decreasing effect over time, but the residuals of the other covariates don't seem to vary over time. This is confirmed by the small p-value for `trt` in the test of the proportional hazards assumption.
```{r}
cox.zph(mycox)
par(mfrow=c(3,2))
plot(cox.zph(mycox))
```
We will therefore try adding into the model an interaction term of treatment with time. We see, as expected, that the interaction term is very significant. However we also see that the proportional hazards model is still rejected, so even with the interaction term the Cox proportional hazards model is not suitable.
```{r}
...
#parmodels[1,] <- c(modpar1$N, modpar1$loglik, modpar1$AIC)
...
parmodels
#anova(modpar1,modpar2,modpar3,modpar4)

pct <- 1:98/100   # The 100th percentile of predicted survival is at +infinity
ptime <- predict(modpar2, newdata=data.frame(trt=0,ager=24,sex=0,nseiz=1,period= 438.4), type='quantile',
                 p=pct, se=TRUE)
...
dfNew <- data.frame(sex=factor(c(0,0)), trt=factor(c(0,1)),ager=84,nseiz=1,period = 438.4, abeeg = 1)
percs <- (1:99)/100
...
matlines(cbind(FWeib$fit[2, ],
               ...
               FWeib$fit[2, ] + 2*FWeib$se.fit[2, ])/30.5, 1-percs, col="red", lwd=2)
legend(x="topright", lwd=2, lty=c(1, 2, 1, 2), col=c("blue", "blue", "red", "red"),
       legend=c("Trt 0 (immediate)", "+- 2*SE", "Trt 0 (deferred)", "+- 2*SE"))
```

Among the accelerated failure time (AFT) models, lognormal model is slightly preferred.
