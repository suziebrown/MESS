...
plot(survfit(Surv(mess$intseiz1,mess$indseiz1, type="right") ~ cut(mess$age, c(-1, 20,40,60,80,100))), main="age", conf.int=T, col=rep(1:10, each=3))
...
```{r}
modpar1 <- survreg(surv1 ~ trt + ager + sex + nseiz + period, mess, dist="exponential")
summary(modpar1)
modpar2 <- survreg(surv1 ~ trt + ager + sex + nseiz + period, mess, dist="weibull")
summary(modpar1)
modpar3 <- survreg(surv1 ~ trt + ager + sex + nseiz + period, mess, dist="lognormal")
summary(modpar3)
modpar4 <- survreg(surv1 ~ trt + ager + sex + nseiz + period, mess, dist="loglogistic")
summary(modpar4)
...
dfNew <- data.frame(sex=c(0,0), trt=c(0,1),ager=24,nseiz=1,period = 438.4)
percs <- (1:99)/100
FWeib <- predict(modpar2, newdata=dfNew, type="quantile", p=percs, se=TRUE)
matplot(cbind(FWeib$fit[1, ],
              FWeib$fit[1, ] - 2*FWeib$se.fit[1, ],
              FWeib$fit[1, ] + 2*FWeib$se.fit[1, ]), 1-percs,
        type="l", main=expression(paste("Weibull-Fit ", hat(S)(t), " with SE")),
