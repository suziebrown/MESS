...
dimnames(t.trt) <- list(treatment=c(0,1))
t.trt
```
...
plot(mfit.trt, conf.int = T, mark.time=F, col =  2:3, xlab = "Years since randomisation", ylab = "Survival probability" ,xscale = 365.25)#,xaxt = 'n')
...
plot(mfit.sex, conf.int = T, mark.time=F, col = c("orange","brown"), xlab = "Years since randomisation",ylab = "Survival probability",xscale = 365.25)#,xaxt = 'n')
#axis(side=1, at=n.years,labels = 1:length(n.years))
...
summary(mycox)$coefficients[,c(1,2,5)]
```
...
summary(mycox2)$coefficients[,c(1,2,5)]
#cox.zph(mycox2)
```
...
summary(mycox3)$coefficients[,c(1,2,5)]
#cox.zph(mycox3)
```
...
summary(mycox4)$coefficients[,c(1,2,5)]
#cox.zph(mycox4)
```
...
modpar.logn <- survreg(surv1 ~ . + trt:nseiz.tr , covars1, dist="lognormal") #using covars before it is defined... or is it covars1?
```
