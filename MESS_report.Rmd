...
kable(cbind(summary(mycox3)$coefficients[,c(1,2,5)],invisible(print(cox.zph(mycox3)))[-nrow(invisible(print(cox.zph(mycox3)))),3]), digits = 3, col.names=c("coef","exp(coef)","p.coef","p.PH"))
#cox.zph(mycox3)
```
...
kable(cbind(summary(mycox4)$coefficients[,c(1,2,5)],invisible(print(cox.zph(mycox4)))[-nrow(invisible(print(cox.zph(mycox4)))),3]), digits = 3, col.names=c("coef","exp(coef)","p.coef","p.PH"))
#cox.zph(mycox4)
```
...
matplot(dat1, 1-percs, type="l", main="Predictive curves for subject with 1 seziure",xlim = c(0,10), xlab="time (years)", ylab="survival probability", lty=c(1,3,3), lwd=2, col="red", cex=0.8)
...
We have not considered the `centre` variable in our models. A good way to include the variation by centre might be to use a frailty model to allow for a random centre effect.
